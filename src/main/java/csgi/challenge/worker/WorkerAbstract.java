...
public abstract class WorkerAbstract<T> implements Worker<T>, Runnable, AutoCloseable {
   ...
   protected abstract void onToken(Token var1) throws Exception;

   public void run() {
      for(Token token = (Token)this.queue.poll(); !this.isClose.get() || token != null; token = (Token)this.queue.poll()) {
